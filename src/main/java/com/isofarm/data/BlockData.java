...
public enum BlockData implements Craftable {
    AIR((byte) 0, (byte) 0, "Air", 0, 0, 0, SoundGroup.SILENT, 0f, true, new MaterialID[]{}),
    DIRT((byte) 1, (byte) 0, "Dirt", true, 100, 0, 0, SoundGroup.SOIL, 0.9f, false, new MaterialID[]{}),
    GRASS((byte) 2, (byte) 0, "Grass", true, 120, 1, 0, SoundGroup.SOIL, 1.0f, false, new Seed[]{new Seed(CropType.WHEAT)}),
    STONE((byte) 3, (byte) 0, "Stone", 150, 2, 0, SoundGroup.HARD, 6.0f, false, new MaterialID[]{}),
    TILLED_DIRT((byte) 4, (byte) 0, "Tilled Dirt", true, false, new MaterialID[]{}, 110, 3, 0, 0, 0, 0, 0, SoundGroup.SOIL, 0.9f),
    VOIDSTONE((byte) 5, (byte) 0, "Voidstone", 999, 4, 0, SoundGroup.HARD, 999999.0f, false, new MaterialID[]{}),
    GLASS((byte) 6, (byte) 0, "Glass", 200, 5, 0, SoundGroup.GLASS, 1.2f, true, new MaterialID[]{}),
    OAK_LOG((byte) 7, (byte) 0, "Oak Log", false, false, new MaterialID[]{MaterialID.WOOD}, 100, 6, 0, 6, 2, 6, 1, SoundGroup.HARD, 1.2f),
    OAK_WOOD((byte) 8, (byte) 0, "Oak Wood", 100, 7, 0, SoundGroup.HARD, 4.0f, false, new MaterialID[]{}),
    OAK_LEAVES((byte) 9, (byte) 0, "Oak Leaves", 100, 8, 0, SoundGroup.SOIL, 1.1f, false, new MaterialID[]{MaterialID.STICK}),
    SNOW((byte) 10, (byte) 0, "Snow", 120, 9, 0, SoundGroup.SNOW, 0.8f, false, new MaterialID[]{}),
    COPPER_ORE((byte) 11, (byte) 1, "Copper Ore", 150, 10, 0, SoundGroup.HARD, 6.0f, false, new MiningComponent[]{new MiningComponent(Tier.COPPER, MaterialID.ORE)}),

    WATER((byte) -1, (byte) -1, "Water", 100, 0, 0, SoundGroup.SILENT, 0f, true, new MaterialID[]{}),
    CROP((byte) -1, (byte) -1, "Crop", -1, 0, 0, SoundGroup.SOIL, 1f, false, new MaterialID[]{});
    ...
    BlockData(byte id, byte row, String name, int value, int tileX, int tileY,
        ...
        this(id, row, name, isTillable, isTransparent, drops, value, tileX, tileY, tileX, tileY,
                tileX, tileY, soundGroup, destroyTime);
    }

    BlockData(byte id, byte row, String name, int value, int tileX, int tileY,
              SoundGroup soundGroup, float destroyTime, boolean isTransparent, Object[] drops) {
        this(id, row, name, false, isTransparent, drops, value, tileX, tileY, tileX,
    ...
    BlockData(byte id, byte row, String name, boolean isTillable, int value, int
        ...
        this(id, row, name, isTillable, isTransparent, drops, value, tileX, tileY, tileX,
                tileY, tileX, tileY, soundGroup, destroyTime);
    }
    ...
    BlockData(byte id, byte row, String name, int value, boolean isTillable, int topTileX,
        ...
        this(id, row, name, isTillable, isTransparent, drops, value, topTileX, topTileY,
                0, 0, 0, 0, soundGroup, destroyTime);
    }

    BlockData(byte id, byte row, String name, boolean isTillable, boolean isTransparent, Object[] drops, int value, int topTileX,
